...
class Hyperparameters:
    ...
    kfac_factor_update_every = int(os.environ.get("KFAC_FACTOR_UPDATE_EVERY", 4))
    kfac_momentum = float(os.environ.get("KFAC_MOMENTUM", 0.95))
    kfac_nesterov = bool(int(os.environ.get("KFAC_NESTEROV", "1")))
    kfac_muon_eps = float(os.environ.get("KFAC_MUON_EPS", 0.04))
    kfac_muon_ns_steps = int(os.environ.get("KFAC_MUON_NS_STEPS", 5))
...
@dataclass
class _KFACExpandConfig:
    damping: float = 1.5e-4
    ema_decay: float = 0.95
    stats_update_every: int = 4
    factor_update_every: int = 4
    momentum: float = 0.95
    nesterov: bool = True
    muon_eps: float = 0.04
    muon_ns_steps: int = 5
...
def _safe_cholesky_2d(matrix: Tensor, max_tries: int = 6, eps: float = 1e-9) -> Tensor:
    eye = torch.eye(matrix.shape[0], device=matrix.device, dtype=matrix.dtype)
    base = float(torch.trace(matrix).item() / max(matrix.shape[0], 1))
    jitter = max(eps, 1e-6 * max(base, eps))
    out = matrix
    for _ in range(max_tries):
        try:
            return torch.linalg.cholesky(out)
        except RuntimeError:
            out = matrix + jitter * eye
            jitter *= 10.0
    return torch.linalg.cholesky(out)


def _adjust_muon_lr(lr: float, shape: tuple[int, int], mode: str) -> float:
    a, b = shape
    if mode in ("", "none", None):
        return lr
    if mode == "original":
        return lr * math.sqrt(max(1.0, float(a) / float(max(b, 1))))
    if mode == "match_rms_adamw":
        return lr * (0.2 * math.sqrt(float(max(a, b))))
    return lr
...
class KFACMuonExpand(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        for module in modules:
            ...
            if weight is None or not isinstance(weight, Tensor) or not weight.requires_grad:
                continue
            if weight.ndim != 2:
                continue
            valid_modules.append(module)
        if not valid_modules:
            raise ValueError("KFACMuonExpand requires at least one trainable 2D weight module.")

        params = [module.weight for module in valid_modules]
        defaults = dict(lr=lr, momentum=float(cfg.momentum))
        super().__init__([{"params": params, "lr": lr, "momentum": float(cfg.momentum)}], defaults)
        ...
        for module in self.modules:
            ...
            out_dim, in_dim = weight.shape[0], weight[0].numel()
            state = self.state[weight]
            state["A"] = torch.eye(in_dim, device=weight.device, dtype=torch.float32)
            state["G"] = torch.eye(out_dim, device=weight.device, dtype=torch.float32)
            state["LA"] = torch.eye(in_dim, device=weight.device, dtype=torch.float32)
            state["LG"] = torch.eye(out_dim, device=weight.device, dtype=torch.float32)
            self._accum[module] = {
                "A_sum": torch.zeros((in_dim, in_dim), device=weight.device, dtype=torch.float32),
                "G_sum": torch.zeros((out_dim, out_dim), device=weight.device, dtype=torch.float32),
                "count": 0.0,
            }
    ...
    def _forward_hook(self, module: nn.Module, inputs, output) -> None:
        ...
        def save_grad(grad: Tensor, m: nn.Module = module, a: Tensor = act_detached) -> None:
            self._accumulate_expand_stats(m, a, grad.detach())

        output.register_hook(save_grad)

    @torch.no_grad()
    def _accumulate_expand_stats(self, module: nn.Module, activations: Tensor, out_grads: Tensor) -> None:
        # Expand setting: flatten batch and sequence dimensions and estimate per-token covariances.
        a2 = activations.float().reshape(-1, activations.shape[-1])
        g2 = out_grads.float().reshape(-1, out_grads.shape[-1])
        if a2.shape[0] == 0 or g2.shape[0] == 0:
            return
        n = int(min(a2.shape[0], g2.shape[0]))
        if n <= 0:
            return
        a2 = a2[:n]
        g2 = g2[:n]
        block = self._accum[module]
        block["A_sum"].add_(a2.transpose(0, 1) @ a2)
        block["G_sum"].add_(g2.transpose(0, 1) @ g2)
        block["count"] += float(n)
    ...
    def _maybe_update_factors(self) -> None:
        ...
        if update_stats:
            ...
            for module in self.modules:
                ...
                if count <= 0.0:
                    continue
                a_batch = _sym_2d(block["A_sum"] / count)
                g_batch = _sym_2d(block["G_sum"] / count)
    ...
    def step(self, closure=None):
        ...
        for module in self.modules:
            ...
            if momentum > 0.0:
                ...
                grad_2d = grad_2d.add(buf, alpha=momentum) if self.cfg.nesterov else buf

            la = state["LA"]
            lg = state["LG"]
            tmp = torch.linalg.solve_triangular(lg, grad_2d, upper=False)
            p_hat = torch.linalg.solve_triangular(la, tmp.transpose(-2, -1), upper=False).transpose(-2, -1)
            q_hat = zeropower_via_newtonschulz5(
                p_hat,
                steps=int(self.cfg.muon_ns_steps),
                eps=float(self.cfg.muon_ns_eps),
            ).float()
            x_hat = -float(self.cfg.muon_eps) * q_hat
            tmp2 = torch.linalg.solve_triangular(lg.transpose(-2, -1), x_hat, upper=True)
            step_2d = torch.linalg.solve_triangular(
                la.transpose(-2, -1),
                tmp2.transpose(-2, -1),
                upper=True,
            ).transpose(-2, -1)
...
def main() -> None:
    ...
    matrix_modules = [
        ...
        and id(module.weight) in matrix_param_ids
    ]
    scalar_params = [
        p
        for name, p in block_named_params
    ...
    if matrix_optimizer_name == "muon":
        ...
        if len(matrix_modules) != len(matrix_params):
    ...
    while True:
        ...
        if hasattr(optimizer_matrix, "set_momentum"):
            optimizer_matrix.set_momentum(muon_momentum)
        else:
            for group in optimizer_matrix.param_groups:
    ...
    close_optimizer = getattr(optimizer_matrix, "close", None)
