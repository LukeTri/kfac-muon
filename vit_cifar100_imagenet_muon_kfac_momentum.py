#!/usr/bin/env python3
"""
Stage-1 ViT benchmark for:
  - plain Muon
  - reduce-style KFAC-Muon

This version supports a few progressively harder image-classification datasets:
  * CIFAR-100 (auto-download supported)
  * Imagenette-320 (auto-download supported)
  * ImageNet / ILSVRC2012 in torchvision-compatible layout

It keeps only what is needed:
  * dataset-specific torchvision loading and transforms
...
Expected ImageFolder layouts:
  root/train/<class_name>/*.jpg
  root/val/<class_name>/*.jpg

Imagenette is additionally accepted at:
  root/imagenette2-320/train, root/imagenette2-320/val

For raw official ImageNet archives, the script will also try torchvision.datasets.ImageNet
when the root contains the standard ILSVRC2012 train/val archives and devkit.
"""
...
DATASET_CHOICES = ("cifar100", "imagenette", "imagenet")
...
def accuracy_topk(logits: torch.Tensor, targets: torch.Tensor, k: int = 1) -> float:
    if logits.ndim != 2:
        raise ValueError(f"Expected logits to have shape [batch, classes], got {tuple(logits.shape)}")
...
@torch.no_grad()
def evaluate(model: nn.Module, loader, device: torch.device, amp_bf16: bool) -> tuple[float, float, float]:
    ...
    for x, y in loader:
        ...
        total_top5 += int(logits.topk(k, dim=1).indices.eq(y[:, None]).any(dim=1).sum().item())
        total += int(y.numel())

    model.train(was_training)
    return (
        total_loss / max(total, 1),
        total_top1 / max(total, 1),
        total_top5 / max(total, 1),
    )
...
def resolve_imagefolder_root(root: str, *, dataset_name: str, nested_dir: Optional[str] = None) -> str:
    root_path = Path(root)
    candidates = [root_path]
    if nested_dir is not None:
        candidates.append(root_path / nested_dir)

    for cand in candidates:
        if (cand / "train").is_dir() and (cand / "val").is_dir():
            return str(cand)

    expected = [f"'{root}/train' and '{root}/val'"]
    if nested_dir is not None:
        expected.append(f"'{root}/{nested_dir}/train' and '{root}/{nested_dir}/val'")

    raise FileNotFoundError(
        f"Could not find {dataset_name} train/val folders under '{root}'. "
        f"Expected one of: {', '.join(expected)}."
    )


def resolve_imagenette_root(root: str) -> str:
    return resolve_imagefolder_root(root, dataset_name="Imagenette", nested_dir=IMAGENETTE_EXTRACTED_DIR)
...
def default_image_size_for_dataset(dataset: str) -> int:
    if dataset == "cifar100":
        return 32
...
def default_patch_size_for_dataset(dataset: str) -> int:
    if dataset == "cifar100":
        return 4
    if dataset in {"imagenette", "imagenet"}:
        return 16
...
def build_imagenet_style_transforms(image_size: int):
...
def build_cifar100_transforms(image_size: int):
    if image_size == 32:
        train_tf = transforms.Compose([
...
def make_data_loaders_from_datasets(
    train_ds,
    val_ds,
    *,
    batch_size: int,
...
def make_imagenette_loaders(
    ...
    train_tf, val_tf = build_imagenet_style_transforms(image_size)
    ...
    train_loader, val_loader = make_data_loaders_from_datasets(
        train_ds,
        val_ds,
        batch_size=batch_size,
    ...
    return train_loader, val_loader, len(train_ds.dataset.classes if isinstance(train_ds, torch.utils.data.Subset) else train_ds.classes)
...
def make_cifar100_loaders(
    ...
    train_tf, val_tf = build_cifar100_transforms(image_size)

    try:
        train_ds = datasets.CIFAR100(root=root, train=True, transform=train_tf, download=download)
        val_ds = datasets.CIFAR100(root=root, train=False, transform=val_tf, download=download)
    except RuntimeError as e:
        if not download:
            raise RuntimeError(
                f"CIFAR-100 was not found under '{root}'. Re-run with --download-cifar100 or pre-download it there."
            ) from e
        raise
    ...
    return train_loader, val_loader, CIFAR100_NUM_CLASSES
...
def make_imagenet_loaders(
    ...
    try:
        ds_root = resolve_imagefolder_root(root, dataset_name="ImageNet")
        train_ds = datasets.ImageFolder(os.path.join(ds_root, "train"), transform=train_tf)
        val_ds = datasets.ImageFolder(os.path.join(ds_root, "val"), transform=val_tf)
    except FileNotFoundError:
        if not hasattr(datasets, "ImageNet"):
            raise RuntimeError(
                "This torchvision build does not expose datasets.ImageNet. "
                "Prepare ImageNet in ImageFolder layout under root/train and root/val."
            )
        try:
            train_ds = datasets.ImageNet(root=root, split="train", transform=train_tf)
            val_ds = datasets.ImageNet(root=root, split="val", transform=val_tf)
        except Exception as e:
            raise RuntimeError(
                f"Could not load ImageNet from '{root}'. Either prepare "
                f"'{root}/train' and '{root}/val' in ImageFolder layout, or place the official "
                f"ILSVRC2012 train/val archives and devkit under that root so torchvision can parse them. "
                f"If raw archive parsing fails, install scipy or do the extraction externally."
            ) from e
    ...
    base_train_ds = train_ds.dataset if isinstance(train_ds, torch.utils.data.Subset) else train_ds
    num_classes = len(getattr(base_train_ds, "classes", [])) or IMAGENET_NUM_CLASSES
...
def make_data_loaders(
    *,
    ...
    num_workers: int,
    image_size: int,
    download_imagenette: bool,
    download_cifar100: bool,
    train_subset: Optional[int],
    val_subset: Optional[int],
    seed: int,
):
    if dataset == "imagenette":
        return make_imagenette_loaders(
            root=root,
            batch_size=batch_size,
            eval_batch_size=eval_batch_size,
            num_workers=num_workers,
...
def validate_train_config(cfg: TrainConfig) -> None:
    if cfg.dataset not in DATASET_CHOICES:
        raise ValueError(f"dataset must be one of {DATASET_CHOICES}, got {cfg.dataset}")
    if cfg.eval_every <= 0:
        raise ValueError(f"eval_every must be positive, got {cfg.eval_every}")
    if cfg.epochs is None and cfg.steps <= 0:
        raise ValueError(f"steps must be positive when epochs is not set, got {cfg.steps}")
    if cfg.epochs is not None and cfg.epochs <= 0:
        raise ValueError(f"epochs must be positive when set, got {cfg.epochs}")
    if cfg.image_size is not None and cfg.image_size <= 0:
        raise ValueError(f"image_size must be positive when set, got {cfg.image_size}")
    if cfg.patch_size is not None and cfg.patch_size <= 0:
        raise ValueError(f"patch_size must be positive when set, got {cfg.patch_size}")
    if cfg.image_size is not None and cfg.patch_size is not None and cfg.image_size % cfg.patch_size != 0:
        raise ValueError(
            f"image_size must be divisible by patch_size, got image_size={cfg.image_size}, patch_size={cfg.patch_size}"
        )
...
def cosine_lr_scale(step: int, total_steps: int, warmup_steps: int, min_lr_scale: float) -> float:
...
def train(cfg: TrainConfig) -> list[dict]:
    ...
    image_size = default_image_size_for_dataset(cfg.dataset) if cfg.image_size is None else cfg.image_size
    patch_size = default_patch_size_for_dataset(cfg.dataset) if cfg.patch_size is None else cfg.patch_size
    if image_size % patch_size != 0:
        raise ValueError(f"image_size must be divisible by patch_size, got image_size={image_size}, patch_size={patch_size}")

    train_loader, val_loader, num_classes = make_data_loaders(
        dataset=cfg.dataset,
        root=cfg.data_root,
        ...
        image_size=image_size,
        download_imagenette=cfg.download_imagenette,
        download_cifar100=cfg.download_cifar100,
    ...
    total_steps = cfg.steps if cfg.epochs is None else max(1, math.ceil(cfg.epochs * len(train_loader)))
    ...
    aux_optim = torch.optim.AdamW(aux_params, lr=aux_lr, betas=cfg.betas, weight_decay=cfg.weight_decay)
    ...
    print(
        f"dataset={cfg.dataset}  num_classes={num_classes}  model={cfg.model_name}  "
        f"image_size={image_size}  patch_size={patch_size}  steps={total_steps}  "
        f"train_batches={len(train_loader)}  val_batches={len(val_loader)}"
    )

    try:
        for step in range(1, total_steps + 1):
            lr_scale = cosine_lr_scale(step, total_steps, cfg.warmup_steps, cfg.min_lr_scale)
            ...
            if step == 1 or step % cfg.eval_every == 0 or step == total_steps:
                elapsed = time.time() - wall_t0
                batch_top1 = accuracy_topk(logits.detach(), y, k=1)
                batch_top5 = accuracy_topk(logits.detach(), y, k=5)
                val_loss, val_top1, val_top5 = evaluate(model, val_loader, device, amp_bf16=cfg.amp_bf16)
                record = dict(
                    step=step,
                    ...
                    batch_acc=float(batch_top1),
                    batch_top1=float(batch_top1),
                    batch_top5=float(batch_top5),
                    val_loss=float(val_loss),
                    val_acc=float(val_top1),
                    val_top1=float(val_top1),
                    val_top5=float(val_top5),
                )
                history.append(record)
                print(
                    f"[{cfg.mode:10s}] {cfg.dataset:10s} "
                    f"step {step:6d}/{total_steps}  epoch {record['epoch']:6.2f}  "
                    f"muon_lr {cur_muon_lr:.3e}  aux_lr {cur_aux_lr:.3e}  "
                    f"batch_loss {record['batch_loss']:.4f}  "
                    f"val_top1 {100.0 * val_top1:5.2f}%  val_top5 {100.0 * val_top5:5.2f}%  "
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(
        description="Stage-1 ViT Muon vs reduce-KFAC-Muon benchmark for CIFAR-100 / Imagenette / ImageNet"
    )
    parser.add_argument("--mode", choices=["muon", "kfac_muon"], default="muon")
    parser.add_argument("--dataset", choices=DATASET_CHOICES, default="cifar100")
    parser.add_argument(
        "--data-root",
        type=str,
        default="./data/cifar100",
        help="For CIFAR-100 this is the torchvision download root. For Imagenette/ImageNet this should contain train/ and val/.",
    )
    parser.add_argument("--download-imagenette", action="store_true")
    parser.set_defaults(download_cifar100=True)
    parser.add_argument("--download-cifar100", dest="download_cifar100", action="store_true")
    parser.add_argument("--no-download-cifar100", dest="download_cifar100", action="store_false")
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="If set, overrides --steps with ceil(epochs * len(train_loader)). Useful when switching between CIFAR-100 and ImageNet.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Defaults to 32 for CIFAR-100 and 224 for Imagenette/ImageNet.",
    )
    parser.add_argument(
        "--patch-size",
        type=int,
        default=None,
        help="Defaults to 4 for CIFAR-100 and 16 for Imagenette/ImageNet.",
    )
    ...
    parser.set_defaults(muon_nesterov=True, kfac_nesterov=True)
    ...
    return TrainConfig(
        ...
        download_cifar100=args.download_cifar100,
