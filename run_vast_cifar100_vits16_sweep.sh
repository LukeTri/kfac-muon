...
KFAC_LR_FIXED="${KFAC_LR_FIXED:-$MUON_LR_FIXED}"
KFAC_DAMPING_BASE="${KFAC_DAMPING_BASE:-2e-5}"
KFAC_EPS_BASE="${KFAC_EPS_BASE:-0.055}"
...
KFAC_LR_LOW="${KFAC_LR_LOW:-5e-4}"
KFAC_LR_HIGH="${KFAC_LR_HIGH:-8e-4}"

if [[ "$PHASE" == "pilot" ]]; then
  EPOCHS="${EPOCHS:-80}"
  BASE_MIXUP_OFF_EPOCH="${BASE_MIXUP_OFF_EPOCH:-60}"
  DEFAULT_SEEDS="${DEFAULT_SEEDS:-12}"
  DEFAULT_CASES="muon_fixed,muon_lr_low,muon_lr_high,kfac_resnet_transfer,kfac_lr_low,kfac_lr_high,kfac_resnet_transfer_lmon,kfac_damp_5e5,kfac_damp_1p5e4,kfac_eps0038,kfac_dp005,kfac_wd005"
elif [[ "$PHASE" == "full" ]]; then
  EPOCHS="${EPOCHS:-200}"
  BASE_MIXUP_OFF_EPOCH="${BASE_MIXUP_OFF_EPOCH:-140}"
  DEFAULT_SEEDS="${DEFAULT_SEEDS:-11 12 13}"
  DEFAULT_CASES="muon_fixed,kfac_resnet_transfer,kfac_eps0038"
else
...
for seed in $SEEDS_STR; do
  # Keep Muon LR fixed and compare KFAC-specific knobs.
  ...
  if contains_case "kfac_damp_1p5e4"; then
    run_train "vits16_c100_kfac_damp1p5e4_e${EPOCHS}_s${seed}" "$seed" \
      --opt kfac_muon --lr "$KFAC_LR_FIXED" \
      --warmup-epochs 5 --drop-path 0.0 \
      --kfac-damping 1.5e-4 --kfac-momentum "$KFAC_MOMENTUM_BASE" --kfac-nesterov \
      --kfac-muon-eps "$KFAC_EPS_BASE" --kfac-muon-lr-adjustment "$KFAC_LR_ADJUSTMENT_BASE" \
      --kfac-stats-update-every "$KFAC_STATS_EVERY_BASE" --kfac-factor-update-every "$KFAC_FACTOR_EVERY_BASE" \
      --no-kfac-lm-adapt-damping --kfac-aux-no-decay
  fi

  if contains_case "kfac_eps0038"; then
    run_train "vits16_c100_kfac_eps0038_e${EPOCHS}_s${seed}" "$seed" \
      --opt kfac_muon --lr "$KFAC_LR_FIXED" \
      --warmup-epochs 5 --drop-path 0.0 \
      --kfac-damping "$KFAC_DAMPING_BASE" --kfac-momentum "$KFAC_MOMENTUM_BASE" --kfac-nesterov \
      --kfac-muon-eps 0.038 --kfac-muon-lr-adjustment "$KFAC_LR_ADJUSTMENT_BASE" \
      --kfac-stats-update-every "$KFAC_STATS_EVERY_BASE" --kfac-factor-update-every "$KFAC_FACTOR_EVERY_BASE" \
      --no-kfac-lm-adapt-damping --kfac-aux-no-decay
  fi
  ...
  if contains_case "kfac_dp005"; then
    run_train "vits16_c100_kfac_dp005_e${EPOCHS}_s${seed}" "$seed" \
      --opt kfac_muon --lr "$KFAC_LR_FIXED" \
      --warmup-epochs 5 --drop-path 0.05 \
      --kfac-damping "$KFAC_DAMPING_BASE" --kfac-momentum "$KFAC_MOMENTUM_BASE" --kfac-nesterov \
      --kfac-muon-eps "$KFAC_EPS_BASE" --kfac-muon-lr-adjustment "$KFAC_LR_ADJUSTMENT_BASE" \
      --kfac-stats-update-every "$KFAC_STATS_EVERY_BASE" --kfac-factor-update-every "$KFAC_FACTOR_EVERY_BASE" \
      --no-kfac-lm-adapt-damping --kfac-aux-no-decay
